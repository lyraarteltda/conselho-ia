    ...
    modal.innerHTML =
      ...
      '<div style="position:relative;background:#13161f;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:28px;width:90%;max-width:440px;">' +
        ...
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          ...
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(245,179,1,.12);border:1px solid rgba(245,179,1,.3);border-radius:10px;color:#f5b301;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">💡 Sugestão</button>' +
        '</div>' +
        ...
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:linear-gradient(180deg,#ffcb3d,#f5b301);color:#211a02;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          x.style.background = x.dataset.type === 'bug' ? 'rgba(239,68,68,.05)' : 'rgba(245,179,1,.05)';
          x.style.borderColor = 'rgba(255,255,255,.08)';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(239,68,68,.1)' : 'rgba(245,179,1,.12)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(239,68,68,.2)' : 'rgba(245,179,1,.3)';
